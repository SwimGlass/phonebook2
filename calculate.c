...
int main(void)
{
    ...
    fp = fopen("avl.txt", "r");
    if (!fp) {
            printf("ERROR opening input file avl.txt\n");
            exit(0);
    }
    double avl_sum_a = 0.0, avl_sum_f = 0.0, avl_a, avl_f;
    for (i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of %d\n", i);
            printf("run 'make run' longer to get enough information\n\n");
    ...
    fprintf(output, "append() %lf %lf %lf %lf\n",orig_sum_a / 100.0, opt_sum_a / 100.0,hash_sum_a / 100.0,avl_sum_a / 100.0);
    fprintf(output, "findName() %lf %lf %lf %lf", orig_sum_f / 100.0, opt_sum_f / 100.0,hash_sum_f /100.0,avl_sum_f / 100.0);
    fclose(output);
    fclose(fp);
    return 0;
}
